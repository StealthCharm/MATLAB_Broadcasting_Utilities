%[text] This function is a broadcasted implementation of isequal for element wise comparisons on complex input objects where the relational operator is not an equivalency check.
function tf = isequal(A, B)
    ...
    for n = 1:numel(tf)
        tf(n) = isequal(A(map(1, n)), B(map(2, n)));
    end
end
